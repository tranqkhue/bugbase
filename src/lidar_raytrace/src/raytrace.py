...
def scan_to_og(data):
	...
	# angle is from x axis , but ROS x_axis heads towards and cv2 x_axis points to the right
	img = cv2.ellipse(img,origin_as_tuple,(lidar_radius_px,lidar_radius_px),angle=0
		,startAngle=angle_min_deg,endAngle=angle_max_deg,color=100,thickness=-1)
	for enum, distance in enumerate(data.ranges):
		...
		cartesian_coordinate = np.array([distance*cos(angle),distance*sin(angle)]) # in metres
	...
	isOccupied_mask = np.where(img == 100,1,0).astype(np.uint8)
	kernel = np.ones((3,3))
	morphed_mask = cv2.morphologyEx(isOccupied_mask,cv2.MORPH_OPEN,kernel)
	noise_and_edge_dots = isOccupied_mask - morphed_mask
	img = np.where(noise_and_edge_dots >0,-1,img).astype(np.int)
	# WARNING!!!
	...
	map_msg = create_occ_grid(img, origin, frame)
...
INF_TO_NUM 		 = rospy.get_param("~inf_to_num", 6)
MAP_TOPIC 	     = rospy.get_param("~map_topic", "/map/scan")
LASER_SCAN_TOPIC = "/scan"
MAP_RESOLUTION   = rospy.get_param("~map_resolution", 0.02)
MAP_SIZE         = rospy.get_param("~map_size", 12)  #Unit: Meter, Shape: Square with center "base_link"
