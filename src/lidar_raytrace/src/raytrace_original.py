...
def scan_to_og(data):
	...
	data.ranges = (np.nan_to_num(data.ranges, nan=NAN_TO_NUM, posinf=INF_TO_NUM)).tolist()
...
INF_TO_NUM 		 = rospy.get_param("~inf_to_num", 5.6)
MAP_TOPIC 	     = rospy.get_param("~map_topic", "/map/scan")
LASER_SCAN_TOPIC = "/scan"
MAP_RESOLUTION   = rospy.get_param("~map_resolution", 0.02)
MAP_SIZE         = rospy.get_param("~map_size", 11.2)  #Unit: Meter, Shape: Square with center "base_link"
